...
const erc=[{name:'balanceOf',type:'function',stateMutability:'view',inputs:[{type:'address'}],outputs:[{type:'uint256'}]},{name:'allowance',type:'function',stateMutability:'view',inputs:[{type:'address'},{type:'address'}],outputs:[{type:'uint256'}]},{name:'decimals',type:'function',stateMutability:'view',inputs:[],outputs:[{type:'uint8'}]}]
...
// facilitator allowance: A must approve the facilitator to move aUSDC via transferFrom
const F=privateKeyToAccount(env.FACILITATOR_PKEY).address
const dec=Number(await pc.readContract({address:AUSDC,abi:erc,functionName:'decimals'}).catch(()=>6))
const allow=await pc.readContract({address:AUSDC,abi:erc,functionName:'allowance',args:[A,F]}).then(v=>formatUnits(v,dec)).catch(()=>'err')
console.log(`facilitator ${F}\naUSDC decimals=${dec}  A->facilitator allowance=${allow}`)
